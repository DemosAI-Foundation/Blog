...
<script>
  ...
  function loadGithubLink() {
    ...
    // Generate a unique suffix using HHMMSS to prevent filename collisions
    ...
    // The template content can be empty or pre-filled as needed
    const template = ``; 
    const encodedTemplate = encodeURIComponent(template);
    
    // Modified filename: YYYY-MM-DD-guest-post-HHMMSS.md
    const filename = `${dateStr}-guest-post-${timeSuffix}.md`;
